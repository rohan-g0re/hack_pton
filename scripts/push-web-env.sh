#!/usr/bin/env bash
set -euo pipefail
# Copies apps/web.env to the web EC2 as ~/hack_pton/.env.
# Run from repo root after setting WEB_HOST in scripts/.deploy.env.
...
HOST="ubuntu@${WEB_HOST:?Set WEB_HOST in scripts/.deploy.env}"
...
echo "Pushing web.env to $HOST (CRLF stripped) ..."
